...
fn main() {
    ...
    let result = map_keypath(person, Person::name(), |name: &String| name.to_uppercase()).unwrap();
    println!("Uppercase name: {}", result);

    // Test filter_by_keypath
    let young_people = filter_by_keypath(people.clone(), Person::age(), |age: &u32| *age < 30).unwrap();
    println!("Young people: {:?}", young_people);

    // Test fold_keypath
    let total_age = fold_keypath(people.clone(), Person::age(), 0, |acc, age: &u32| acc + *age).unwrap();
    println!("Total age: {}", total_age);

    // Test find_by_keypath
    let found = find_by_keypath(people.clone(), Person::age(), |age: &u32| *age == 30).unwrap();
    ...
    let (young, old) = partition_by_keypath(people.clone(), Person::age(), |age: &u32| *age < 30).unwrap();
    println!("Young: {:?}, Old: {:?}", young, old);

    // Test map_keypath_collection
    let names = map_keypath_collection(&people, Person::name(), |name: &String| name.to_uppercase()).unwrap();
    println!("All names uppercase: {:?}", names);

    // Test group_by_keypath
    let grouped = group_by_keypath(&people, Person::age(), |&age| {
    ...
    sort_by_keypath(&mut sorted_people, Person::age(), |a, b| a.cmp(b)).unwrap();
    ...
    let combined = zip_with_keypath(&people, &people2, Person::name(), Person::name(), |name1, name2| {
        format!("{} & {}", name1, name2)
    }).unwrap();
